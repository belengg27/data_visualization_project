...
# Define the UI
ui <- fluidPage(
  numericInput("min_price", "Minimum Price:", min = min(data$price), max = max(data$price), value = min(data$price)),
  numericInput("max_price", "Maximum Price:", min = min(data$price), max = max(data$price), value = max(data$price)),
...
server <- function(input, output) {
  ...
  filtered_data <- reactive({
    data[data$price >= input$min_price & data$price <= input$max_price,]
  })
